...
public:
	Texture(const std::string& path, const int texture_type = DIFFUSE, GLenum pixel_format = GL_RGBA, GLenum pixel_type = GL_UNSIGNED_BYTE,
		GLenum min_filter = GL_LINEAR, GLenum mag_filter = GL_LINEAR, GLenum wrap_s = GL_REPEAT, GLenum wrap_t = GL_REPEAT);
	Texture(unsigned char* bytes, int width, int height, const int texture_type = DIFFUSE, GLenum pixel_format = GL_RGB, GLenum pixel_type = GL_UNSIGNED_BYTE,
	...
	Texture(GLuint textureID, int width, int height, const int texture_type = DIFFUSE) : m_RendererID(textureID), m_TextureType(texture_type), m_Width(width), m_Height(height) {}; 
	...
	inline int GetType() const { return m_TextureType; }
	...
	static const int SHADOWMAP = 3;
	static const int NORMALMAP = 4;
	static const int DISPLACEMENTMAP = 5;
